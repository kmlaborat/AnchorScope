...
fn nested_read_creates_single_hierarchical_chain() {
    ...
    let label_out =
        run_anchorscope(&["label", "--name", &label_name, "--true-id", &level1_true_id]);
    assert!(
        ...
        if let Ok(entries) = std::fs::read_dir(&anchorscope_dir) {
            for entry in entries.flatten() {
                if entry.file_type().map(|t| t.is_dir()).unwrap_or(false) {
                    ...
                    let content_path = anchorscope_dir
                        .join(file_hash_str)
                        .join(&level1_true_id)
                        .join("content");
                    ...
                    if let Ok(file_dir_entries) = std::fs::read_dir(&file_dir) {
                        for parent_entry in file_dir_entries.flatten() {
                                ...
                                .unwrap_or(false)
                            {
                                ...
                                let nested_content_path = anchorscope_dir
                                    .join(file_hash_str)
    ...
    let nested_level2_dir = anchorscope_dir
        .join(&file_hash)
        .join(&level1_true_id)
        .join(&level2_true_id);
    ...
    assert!(
        nested_level2_dir.join("..").join("content").exists(),
        "level 1 buffer should exist"
    );

    // Level 2 should exist ONLY as nested under level 1
    assert!(
        nested_level2_dir.join("content").exists(),
        "level 2 nested buffer should exist"
    );
    assert!(
        nested_level2_dir.join("metadata.json").exists(),
        "level 2 nested metadata should exist"
    );

    // Level 2 should NOT exist as flat directory (this is what we're testing for)
    assert!(
        !flat_level2_dir.exists(),
        "level 2 should NOT exist as flat directory (orphan prevention)"
    );
...
fn three_level_nesting_write_cleans_up_correctly() {
    ...
    assert!(
        label_out1.status.success(),
        "label1 failed: {}",
        String::from_utf8_lossy(&label_out1.stderr)
    );
    ...
    assert!(
        label_out2.status.success(),
        "label2 failed: {}",
        String::from_utf8_lossy(&label_out2.stderr)
    );
    ...
    assert!(
        label_out3.status.success(),
        "label3 failed: {}",
        String::from_utf8_lossy(&label_out3.stderr)
    );
        ...
        if let Ok(entries) = std::fs::read_dir(&anchorscope_dir) {
            for entry in entries.flatten() {
                if entry.file_type().map(|t| t.is_dir()).unwrap_or(false) {
                    ...
                    let file_hash_str = file_hash.to_string_lossy();
                    let file_hash_str = file_hash_str.as_ref();
                    let level1_path = anchorscope_dir
                        .join(file_hash_str)
                        .join(&level1_true_id)
                        .join("content");
    ...
    let level2_path = anchorscope_dir
        .join(&file_hash)
        .join(&level1_true_id)
        .join(&level2_true_id);
    let level3_path = anchorscope_dir
        .join(&file_hash)
        .join(&level1_true_id)
        .join(&level2_true_id)
        .join(&level3_true_id);
    ...
    assert!(
        level2_path.join("content").exists(),
        "level 2 should exist as nested"
    );
    assert!(
        level3_path.join("content").exists(),
        "level 3 should exist as nested"
    );
