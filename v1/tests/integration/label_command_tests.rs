...
fn test_label_command_success() {
    ...
    let (_temp_dir, file_path) = create_temp_file("fn main() {\n    println!(\"Hello\");\n}\n");
...
fn test_label_unknown_internal() {
    ...
    let output = run_anchorscope(&["label", "--name", "test", "--true-id", "0000000000000000"]);
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("IO_ERROR: buffer metadata for true_id")
            || stderr.contains("IO_ERROR: file not found")
            || stderr.contains("IO_ERROR: anchor metadata for hash")
    );
}
...
fn test_label_name_collision() {
    ...
    let out_label1 = run_anchorscope(&["label", "--name", "func", "--true-id", &label1]);
    ...
    let out_label2 = run_anchorscope(&["label", "--name", "func", "--true-id", &label2]);
